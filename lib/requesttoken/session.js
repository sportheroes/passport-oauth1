...
SessionStore.prototype.set = function(req, token, tokenSecret, cb) {
  ...
  // Withings API has some troubles and works only when tokens are reverted
  // so we need to put them in this particular order for withings only
  if (req.path.toString().search('withings') >= 0) {
    req.session[this._key].oauth_token = tokenSecret;
    req.session[this._key].oauth_token_secret = token;
  } else {
