...
function OAuthStrategy(options, verify) {
  ...
  this._getOAuthRequestToken = options.getOAuthRequestToken;
  this._stored = options.stored;
  this._getOAuthAccessToken = options.getOAuthAccessToken;
...
OAuthStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.oauth_token) {
    ...
    function loaded(err, oauthTokenSecret, state) {
      ...
      const getOAuthAccessToken = this._getOAuthAccessToken || self._oauth.getOAuthAccessToken.bind(oauthToken, oauthTokenSecret, oauthVerifier);

      getOAuthAccessToken(oauthToken, oauthTokenSecret, oauthVerifier, function(err, token, tokenSecret, params) {
    ...
    const getOAuthRequestToken = this._getOAuthRequestToken || this._oauth.getOAuthRequestToken.bind(this._oauth);
    
    getOAuthRequestToken(params, function(err, token, tokenSecret, params) {
      if (!token || !tokenSecret) { return self.error(self._createOAuthError('token or tokenSecret are not set', err)); }
      ...
      const mainStore = self._stored ? self._stored(self, tokenSecret) : stored;

      try {
        var arity = self._requestTokenStore.set.length;
        if (arity == 5) {
          self._requestTokenStore.set(req, token, tokenSecret, meta, mainStore);
        } else { // arity == 4
          self._requestTokenStore.set(req, token, tokenSecret, mainStore);
        }
      } catch (ex) {
        return self.error(ex);
      }
